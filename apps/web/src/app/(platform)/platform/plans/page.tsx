...
export default async function Page() {
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <div><div className="lf-eyebrow">Commercial</div><h1 style={{ margin: '8px 0 0' }}>Subscription plans</h1></div>
      <PlanForm />
      <WorkspaceTable
        ...
        rows={plans.map((plan) => [
          plan.name,
          plan.code,
          (plan.planModules.length ? plan.planModules.filter((module) => module.enabled).map((module) => module.module) : plan.modules).join(' + '),
