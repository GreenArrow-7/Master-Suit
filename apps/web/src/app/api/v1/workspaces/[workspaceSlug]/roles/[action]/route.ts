...
import {
  assignRole, createRole, deleteRole, listRoles, permissionMatrix,
  revokeRoleAssignment, roleAssignmentHistory, updatePermissionMatrix, updateRole,
...
export const GET = route(
  { module: 'roles', productModule: 'HRMS', action: 'VIEW', params: paramsSchema, query: z.object({ roleId: z.string().max(64).optional(), membershipId: z.string().max(64).optional(), limit: z.coerce.number().int().min(1).max(500).optional() }).passthrough() },
  async ({ ctx, params, query }) => {
    await requireWorkspace(ctx, params.workspaceSlug);
    switch (params.action) {
      case 'roles': return listRoles(ctx);
      case 'matrix': {
        if (!query.roleId) throw new Error('roleId is required.');
        return permissionMatrix(ctx, query.roleId);
      }
      case 'history': return roleAssignmentHistory(ctx, { roleId: query.roleId, membershipId: query.membershipId, limit: query.limit });
...
export const POST = route(
  { module: 'roles', productModule: 'HRMS', action: 'MANAGE_CONFIGURATION', params: paramsSchema, body: z.record(z.string(), z.unknown()) },
  async ({ ctx, params, body }) => {
    ...
    switch (params.action) {
      ...
      case 'matrix-update': {
        const input = z.object({
          roleId: id,
          changes: z.array(z.object({ permissionId: id, granted: z.coerce.boolean(), scope })).min(1).max(500),
        }).parse(body);
