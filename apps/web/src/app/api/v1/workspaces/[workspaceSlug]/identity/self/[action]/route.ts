...
import {
  ...
  regenerateRecoveryCodes, twoFactorStatus,
 ...
 *
 * Gated on `hrms:VIEW` rather than a `users` permission on purpose. Employees
 * created through the HR flow hold hrms but no users permission at all, so
 * gating self-service on `users` would leave exactly the people who most need to
 * change a temporary password unable to do it.
 */
const paramsSchema = z.object({
  workspaceSlug: z.string().min(2).max(64),
  action: z.enum([
    'password-status', 'two-factor-status',
    'password-change', 'two-factor-begin', 'two-factor-confirm',
    'two-factor-disable', 'two-factor-recovery-regenerate',
  ]),
});

export const GET = route(
  { module: 'hrms', productModule: 'HRMS', action: 'VIEW', params: paramsSchema },
  async ({ ctx, params }) => {
    await requireWorkspace(ctx, params.workspaceSlug);
    switch (params.action) {
      case 'password-status': return { mustChangePassword: await mustChangePassword(ctx) };
      case 'two-factor-status': return twoFactorStatus(ctx);
...
export const POST = route(
  { module: 'hrms', productModule: 'HRMS', action: 'VIEW', params: paramsSchema, body: z.record(z.string(), z.unknown()) },
  async ({ ctx, params, body }) => {
    await requireWorkspace(ctx, params.workspaceSlug);

    switch (params.action) {
      case 'password-change': {
        const input = z.object({ currentPassword: z.string().min(1).max(512), newPassword: z.string().min(8).max(512) }).parse(body);
        return changeOwnPassword(ctx, input.currentPassword, input.newPassword);
      }
      case 'two-factor-begin': return beginTotpEnrolment(ctx);
