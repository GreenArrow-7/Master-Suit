...
import { assertBookingInScope } from '@/services/money/collections';
...
export const GET = route(
  ...
  async ({ ctx, query }) =>
    withTx(ctx.tenantId, async (tx) => {
      await assertBookingInScope(tx, ctx, 'agencyfee', 'CREATE', query.bookingId);
      return previewAmendment(tx, ctx.tenantId, query.bookingId, new Prisma.Decimal(query.proposedFee));
    }),
);
