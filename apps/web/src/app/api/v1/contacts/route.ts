...
export const GET = route(
  { module: 'contacts', productModule: 'SALES', action: 'VIEW', query: listQuery },
  async ({ ctx, query }) => {
    ...
    const where = mergeWhere(
      scopeWhere,
      tree ? compileFilterTree('CONTACT', tree, ctx) : null,
...
export const POST = route(
  { module: 'contacts', productModule: 'SALES', action: 'CREATE', body: createBody, auditEvent: 'RECORD_CREATED' },
  async ({ ctx, body }) => {
    const rules = await loadFieldRules(ctx, 'CONTACT');
