...
export const GET = route(
  { module: 'contacts', action: 'VIEW', params },
  async ({ ctx, params }) => {
    ...
    const contact = await prisma.contact.findFirst({
      ...
      include: { account: { select: { id: true, name: true } }, owner: { select: { id: true, fullName: true, email: true } } },
    });
...
export const PATCH = route(
  { module: 'contacts', action: 'EDIT', params, body: patchBody, auditEvent: 'RECORD_UPDATED' },
...
export const DELETE = route(
  { module: 'contacts', action: 'DELETE', params, auditEvent: 'RECORD_DELETED' },
  async ({ ctx, params }) => {
