...
const TABS = [['All', ''], ['Overdue', 'overdue'], ['Today', 'today'], ['Upcoming', 'upcoming']] as const;
const PRIORITY_TONE: Record<string, Tone> = { URGENT: 'vermillion', HIGH: 'wine', MEDIUM: 'brass', LOW: 'slate' };
...
export default async function FollowUpsPage({ searchParams }: { searchParams: Promise<{ due?: string }> }) {
  ...
  return (
    <>
      <ListHeader title="Follow-ups" count={rows.length} capped={rows.length === 100} 
        actions={can(ctx, 'settings', 'MANAGE_CONFIGURATION') && <ColumnEditor object="FOLLOWUP" current={columns.map((c) => c.key)} />}
      />

      <nav className="lf-tabs" style={{ marginBottom: 'var(--lf-space-4)' }} aria-label="Due filter">
        {TABS.map(([label, key]) => (
          <SalesLink key={label} className="lf-tab" href={key ? `/follow-ups?due=${key}` : '/follow-ups'}
             aria-selected={(params.due ?? '') === key} role="tab">
            {label}
          </SalesLink>
        ))}
      </nav>
      ...
      {rows.length === 0 ? (
        <div className="lf-card"><EmptyState title="No follow-ups" description="Follow-up tasks will appear here." /></div>
      ) : (
