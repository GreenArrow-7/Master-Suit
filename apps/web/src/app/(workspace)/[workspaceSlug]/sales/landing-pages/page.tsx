...
const TABS = [['All', ''], ['Published', 'PUBLISHED'], ['Draft', 'DRAFT']] as const;
...
export default async function LandingPagesPage({ searchParams }: { searchParams: Promise<{ state?: string }> }) {
  ...
  const rows = await prisma.landingPage.findMany({
    ...
    select: {
      id: true, name: true, slug: true, state: true, customDomain: true,
      visitCount: true, submissionCount: true, publishedAt: true, createdAt: true,
    },
    take: 50,
  });
  ...
  return (
    <>
      ...
      <nav className="lf-tabs" style={{ marginBottom: 'var(--lf-space-4)' }} aria-label="State filter">
        {TABS.map(([label, key]) => (
          <SalesLink key={label} className="lf-tab" href={key ? `/landing-pages?state=${key}` : '/landing-pages'}
      ...
      {rows.length === 0 ? (
        <div className="lf-card">
          <EmptyState title="No landing pages yet" description="Create a landing page to capture leads from campaigns." />
        </div>
      ) : (
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td style={{ color: 'var(--lf-ink-3)', fontFamily: 'monospace', fontSize: 'var(--lf-text-sm)' }}>/{r.slug}</td>
                    ...
                    <td style={{ color: 'var(--lf-ink-3)', fontSize: 'var(--lf-text-sm)' }}>
                      {r.publishedAt ? new Date(r.publishedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                    </td>
                  </tr>
                );
              })}
