...
export default async function AccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const account = await prisma.account.findFirst({
    ...
    include: {
      ...
      contacts: { where: { deletedAt: null }, take: 10, select: { id: true, fullName: true, jobTitle: true, email: true } },
      opportunities: { where: { deletedAt: null }, take: 10, select: { id: true, name: true, status: true, amount: true, currency: true } },
    },
  });
  if (!account) notFound();
    ...
    <>
      ...
      <header style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--lf-space-4)', margin: 'var(--lf-space-3) 0 var(--lf-space-4)' }}>
        <span className="lf-avatar" style={{ width: 44, height: 44, fontSize: 'var(--lf-text-base)' }}>
          {account.name.split(' ').slice(0, 2).map((p) => p[0]).join('')}
        </span>
        <div style={{ minWidth: 0 }}>
          <h1 className="lf-h1" style={{ fontSize: 'var(--lf-text-2xl)' }}>{account.name}</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--lf-space-3)', marginTop: 4, flexWrap: 'wrap' }}>
            <span className="lf-num" style={{ fontSize: 'var(--lf-text-xs)', color: 'var(--lf-ink-3)' }}>{account.reference}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 320px) minmax(0, 1fr)', gap: 'var(--lf-space-4)', alignItems: 'start' }}>
        <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
          ...
          <dl style={{ display: 'grid', gap: 'var(--lf-space-3)', margin: 0 }}>
            {([
              ...
              ['Created', account.createdAt.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })],
        ...
        <div style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
          <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
            <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-4)' }}>Contacts</div>
            {account.contacts.length === 0 ? (
              <p style={{ color: 'var(--lf-ink-3)', fontSize: 'var(--lf-text-sm)', margin: 0 }}>No contacts linked yet.</p>
            ) : (
              account.contacts.map((c) => (
                <div key={c.id} style={{ padding: '6px 0', borderBottom: '1px solid var(--lf-line)' }}>
                  <SalesLink href={`/contacts/${c.id}`} style={{ fontWeight: 500 }}>{c.fullName}</SalesLink>
                  <span style={{ marginLeft: 8, fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-3)' }}>{c.jobTitle}</span>
                </div>
              ))
            )}
          </section>
          ...
          <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
            <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-4)' }}>Opportunities</div>
            {account.opportunities.length === 0 ? (
              <p style={{ color: 'var(--lf-ink-3)', fontSize: 'var(--lf-text-sm)', margin: 0 }}>No opportunities linked yet.</p>
            ) : (
              account.opportunities.map((o) => (
                <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--lf-line)' }}>
                  <SalesLink href={`/opportunities/${o.id}`} style={{ fontWeight: 500 }}>{o.name}</SalesLink>
                  <Badge value={o.status} />
                  <span style={{ marginLeft: 'auto', fontFamily: 'var(--lf-font-mono)', fontSize: 'var(--lf-text-sm)' }}>
