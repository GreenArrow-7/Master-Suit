...
const OUTCOME_TONE: Record<string, Tone> = {
  CONNECTED: 'viridian', INTERESTED: 'viridian', QUALIFIED: 'viridian', CONVERTED: 'viridian',
  NO_ANSWER: 'slate', BUSY: 'brass', VOICEMAIL: 'brass',
  NOT_INTERESTED: 'vermillion', WRONG_NUMBER: 'vermillion',
...
export default async function CallDetailPage({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 'var(--lf-space-5)' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-3)' }}>
            ...
            {call.startedAt && ` · ${call.startedAt.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}`}
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 'var(--lf-space-4)', marginBottom: 'var(--lf-space-5)' }}>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-eyebrow">Duration</div>
          <div className="lf-num" style={{ fontSize: 'var(--lf-text-2xl)', fontWeight: 700, marginTop: 4 }}>{fmtDuration(call.durationSecs)}</div>
        </div>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-eyebrow">Status</div>
          <div style={{ fontSize: 'var(--lf-text-xl)', fontWeight: 700, marginTop: 4 }}>{call.status.toLowerCase()}</div>
        </div>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-eyebrow">Consent</div>
          <div style={{ marginTop: 4 }}><Badge tone={hasConsent ? 'viridian' : 'vermillion'}>{hasConsent ? 'given' : 'none'}</Badge></div>
        </div>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-eyebrow">Transcript</div>
          <div style={{ marginTop: 4 }}><Badge tone={call.transcript ? 'viridian' : 'slate'}>{call.transcript ? `${call.transcript.wordCount} words` : 'none'}</Badge></div>
        </div>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-eyebrow">AI Analysis</div>
          <div style={{ marginTop: 4 }}><Badge tone={analysis?.status === 'COMPLETED' ? 'viridian' : analysis?.status === 'FAILED' ? 'vermillion' : 'slate'}>{analysis?.status?.toLowerCase() ?? 'none'}</Badge></div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 'var(--lf-space-4)' }}>
        ...
        <div style={{ display: 'grid', gap: 'var(--lf-space-4)', alignContent: 'start' }}>
          <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
            <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Notes</div>
            <p style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)', whiteSpace: 'pre-wrap' }}>
              {call.notes || 'No notes recorded.'}
            </p>
          </section>

          <CallActions callId={params.id} hasConsent={!!hasConsent} callStatus={call.status} hasTranscript={!!call.transcript} hasAnalysis={!!analysis} />

          {/* Consent details */}
          {call.consent && (
            <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
              <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Consent Details</div>
              <dl style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 12px', margin: 0 }}>
                ...
                {call.consent.withdrawnAt && (
                  <>
                    ...
                    <dd style={{ margin: 0, color: 'var(--lf-vermillion)' }}>{call.consent.withdrawnAt.toLocaleString('en-GB')}</dd>
                  </>
                )}
              </dl>
            </section>
          ...
          {call.recording && (
            <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
              <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Recording</div>
              <dl style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 12px', margin: 0 }}>
          ...
          <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
            <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Follow-ups</div>
            {followUps.length === 0 ? (
              <p style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-3)' }}>No follow-ups from this call.</p>
            ) : (
              <ul style={{ margin: 0, paddingLeft: 'var(--lf-space-5)', fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)' }}>
                {followUps.map((f) => (
                  <li key={f.id} style={{ marginBottom: 4 }}>
                    <span style={{ fontWeight: 500 }}>{f.title}</span>
                    <span style={{ color: 'var(--lf-ink-3)', marginLeft: 8 }}>due {f.dueAt.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
                    {' '}<Badge tone={f.status === 'COMPLETED' ? 'viridian' : f.status === 'OPEN' ? 'brass' : 'slate'}>{f.status.toLowerCase()}</Badge>
                  </li>
                ))}
              </ul>
            )}
        ...
        <div style={{ display: 'grid', gap: 'var(--lf-space-4)', alignContent: 'start' }}>
          ...
          {audits.length > 0 && (
            <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
              <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Call Audits</div>
              {audits.map((a) => {
                const pct = a.maxScore ? Math.round(((a.overallScore ?? 0) / a.maxScore) * 100) : null;
                const scores = (a.criteriaScores as any[] | null) ?? [];
                return (
                  <div key={a.id} style={{ borderBottom: '1px solid var(--lf-line)', paddingBottom: 'var(--lf-space-3)', marginBottom: 'var(--lf-space-3)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <Badge tone={a.status === 'COMPLETED' ? 'viridian' : a.status === 'FAILED' ? 'vermillion' : 'brass'}>
                        {a.status.toLowerCase()}
                      </Badge>
                      {pct !== null && (
                        <span className="lf-num" style={{ fontSize: 'var(--lf-text-lg)', fontWeight: 700, color: pct >= 70 ? 'var(--lf-viridian)' : pct >= 40 ? 'var(--lf-brass)' : 'var(--lf-vermillion)' }}>
                          {pct}%
                        </span>
                      )}
                    </div>
                      ...
                      <div style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)' }}>
                        {scores.map((s: any, i: number) => (
                          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 0' }}>
                            <span>{s.met ? '✓' : '✗'} {s.label}</span>
                            <span className="lf-num">{s.score}/{s.maxScore}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {((a.suggestions as string[]) ?? []).length > 0 && (
                      <div style={{ marginTop: 8 }}>
                        <div style={{ fontSize: 'var(--lf-text-2xs)', fontWeight: 500, color: 'var(--lf-ink-3)', marginBottom: 4 }}>Coaching</div>
                        <ul style={{ margin: 0, paddingLeft: 'var(--lf-space-4)', fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)' }}>
                          {((a.suggestions as string[]) ?? []).map((s, i) => <li key={i}>{s}</li>)}
                        </ul>
                      </div>
                    )}
