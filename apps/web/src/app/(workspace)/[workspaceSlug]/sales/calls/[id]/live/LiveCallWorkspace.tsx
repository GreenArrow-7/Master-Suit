...
export default function LiveCallWorkspace({
  ...
  /** What the connected vendor can do for this call — see liveAudio.ts. */
  liveAudio?: LiveAudio;
  vendor?: string | null;
  /** The vendor has already been asked to place it. */
  placed?: boolean;
  /** Consent is on file, so the call can be placed with streaming. */
  consented?: boolean;
}) {
  ...
  // A call the vendor is carrying is displayed, never simulated.
  const vendorMode = transport === 'vendor' || placed || placedNow;
  const canPlace = liveAudio !== 'no-vendor' && Boolean(call.recipientNumber);
  ...
  /**
   * The AI-assisted call: the vendor rings this seller's phone first and
   * bridges the customer, so the conversation is a normal phone call on the
   * handset while its audio — at the vendor, never on the device — feeds the
   * engine. Consent is affirmed and recorded before the vendor is asked,
   * because streaming is processing the conversation, same as recording it.
   */
  async function placeCall() {
    setPlaceError('');
    if (!consented) {
      const affirmed = window.confirm(
        'Confirm the customer has already agreed to a recorded, AI-assisted call. This is recorded against the call before it is placed.',
      );
      if (!affirmed) return;
    }
    setPlacing(true);
    try {
      if (!consented) {
        const consent = await fetch(`/api/v1/calls/${call.id}/consent`, {
          ...
          body: JSON.stringify({ consentGiven: true, method: 'PRE_AUTHORIZED' }),
        });
        if (!consent.ok && consent.status !== 409) throw new Error('Could not record consent.');
      }
      const res = await fetch(`/api/v1/calls/${call.id}/dial`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: '{}',
      });
      if (!res.ok) {
        ...
        throw new Error(data.errors?.[0]?.message ?? data.detail ?? 'The provider could not place the call.');
      }
      setPlacedNow(true);
      start();
    } catch (err) {
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
      <div className="lf-alert" role="note">
        {vendorMode || liveAudio !== 'no-vendor'
          ? LIVE_AUDIO_NOTICE[liveAudio](vendorLabel(vendor))
      ...
      <div style={{ display: 'flex', gap: 'var(--lf-space-2)', flexWrap: 'wrap' }}>
            ...
            <>
              {canPlace && (
                <button
                  ...
                  disabled={placing}
                  title="Rings your phone first, then the customer; guidance appears here"
                >
                  {placing ? 'Placing…' : 'Place call with AI assistance'}
                </button>
              )}
              <button className={canPlace ? 'lf-btn lf-btn--secondary' : 'lf-btn'} onClick={start}>
        ...
        {placeError && (
          <span
            role="alert"
            style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-vermillion)', alignSelf: 'center' }}
          >
            {placeError}
            {call.recipientNumber && (
              <>
                {' '}
                <a href={`tel:${call.recipientNumber}`}>Call from this phone instead (no AI assistance)</a>
              </>
            )}
          </span>
        )}
        {phase === 'live' && (
          <>
            ...
            <button className="lf-btn lf-btn--danger" onClick={endCall}>
              {vendorMode ? 'Stop watching' : 'End call'}
            </button>
          </>
        )}
        {phase === 'done' && (
