...
export default function ContactForm({ accounts }: { accounts: { id: string; name: string }[] }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.detail ?? 'Could not create this contact.'); return; }
  ...
  return (
    <form onSubmit={submit} style={{ display: 'grid', gap: 'var(--lf-space-4)' }} noValidate>
      ...
      <div style={{ display: 'flex', gap: 'var(--lf-space-3)', marginTop: 'var(--lf-space-2)' }}>
        <button className="lf-btn" type="submit" disabled={busy}>{busy ? 'Creating…' : 'Create contact'}</button>
        <SalesLink className="lf-btn lf-btn--secondary" href="/contacts">Cancel</SalesLink>
      </div>
    </form>
  );
}
