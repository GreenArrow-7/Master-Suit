    ...
    <>
      ...
      <header style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--lf-space-4)', margin: 'var(--lf-space-3) 0 var(--lf-space-4)' }}>
        <span className="lf-avatar" style={{ width: 44, height: 44, fontSize: 'var(--lf-text-base)' }}>
          {contact.fullName.split(' ').slice(0, 2).map((p) => p[0]).join('')}
        </span>
        <div style={{ minWidth: 0 }}>
          <h1 className="lf-h1" style={{ fontSize: 'var(--lf-text-2xl)' }}>{contact.fullName}</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--lf-space-3)', marginTop: 4, flexWrap: 'wrap' }}>
            <span className="lf-num" style={{ fontSize: 'var(--lf-text-xs)', color: 'var(--lf-ink-3)' }}>{contact.reference}</span>
      ...
      <section className="lf-card" style={{ padding: 'var(--lf-space-5)', maxWidth: 420 }}>
        ...
        <dl style={{ display: 'grid', gap: 'var(--lf-space-3)', margin: 0 }}>
          {([
            ...
            ['Created', contact.createdAt.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })],
