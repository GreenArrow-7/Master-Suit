...
export default function OutcomeActions({
  opportunityId, lossReasons,
}: { opportunityId: string; lossReasons: { id: string; name: string }[] }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', gap: 'var(--lf-space-2)' }}>
      ...
      {asking && (
        <form
          className="lf-card"
          style={{ position: 'absolute', right: 0, top: '100%', marginTop: 6, zIndex: 20, width: 300, padding: 'var(--lf-space-4)', boxShadow: 'var(--lf-shadow-2)', textAlign: 'left' }}
          onSubmit={(event) => {
        ...
        >
          ...
          {lossReasons.length > 0 && (
            <select className="lf-input" name="lossReasonId" defaultValue="" style={{ marginTop: 8 }}>
              ...
              {lossReasons.map((reason) => <option key={reason.id} value={reason.id}>{reason.name}</option>)}
            </select>
          )}
          <textarea className="lf-input" name="lossNotes" rows={3} maxLength={1000} placeholder="Notes (optional)" style={{ marginTop: 8 }} />
          {error && <p style={{ color: 'var(--lf-vermillion)', fontSize: 'var(--lf-text-2xs)', marginTop: 6 }}>{error}</p>}
          <div style={{ display: 'flex', gap: 8, marginTop: 'var(--lf-space-3)' }}>
            <button className="lf-btn lf-btn--sm" type="submit" disabled={busy}>{busy ? 'Saving…' : 'Confirm lost'}</button>
            <button className="lf-btn lf-btn--sm lf-btn--secondary" type="button" onClick={() => setAsking(false)}>Cancel</button>
          </div>
        </form>
      )}

      {error && !asking && <span style={{ color: 'var(--lf-vermillion)', fontSize: 'var(--lf-text-2xs)', alignSelf: 'center' }}>{error}</span>}
    </div>
  );
}
