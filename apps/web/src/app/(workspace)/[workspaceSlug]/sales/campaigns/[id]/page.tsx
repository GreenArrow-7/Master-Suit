...
export default async function CampaignDetailPage({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
  ...
  const eligible = await prisma.lead.count({
    where: {
      tenantId: ctx.tenantId, campaignId: params.id, deletedAt: null,
  ...
  return (
    <>
      ...
      {/* Metrics row */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 'var(--lf-space-4)', marginBottom: 'var(--lf-space-5)' }}>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-num" style={{ fontSize: 'var(--lf-text-2xl)', fontWeight: 700 }}>{leadCount}</div>
          <div style={{ fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-3)' }}>Leads</div>
        </div>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-num" style={{ fontSize: 'var(--lf-text-2xl)', fontWeight: 700 }}>{members.length}</div>
          <div style={{ fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-3)' }}>Members</div>
        </div>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-num" style={{ fontSize: 'var(--lf-text-2xl)', fontWeight: 700 }}>{scripts.length}</div>
          <div style={{ fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-3)' }}>Scripts</div>
        </div>
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div className="lf-num" style={{ fontSize: 'var(--lf-text-2xl)', fontWeight: 700 }}>{talkingPoints.length}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 'var(--lf-space-4)' }}>
        ...
        <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
          <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Call Scripts</div>
          {scripts.length === 0 ? (
            <p style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-3)' }}>No scripts defined. Add scripts via the API.</p>
          ) : (
            <div style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
              {scripts.map((s) => (
                <div key={s.id} style={{ borderBottom: '1px solid var(--lf-line)', paddingBottom: 'var(--lf-space-3)' }}>
                  <div style={{ fontWeight: 600, fontSize: 'var(--lf-text-sm)', marginBottom: 4 }}>
                    {s.title} {s.isDefault && <Badge tone="viridian">default</Badge>}
                  </div>
                  <div style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)', whiteSpace: 'pre-wrap' }}>{s.content}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
          <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
            <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Required Talking Points</div>
              ...
              <ul style={{ margin: 0, paddingLeft: 'var(--lf-space-5)' }}>
                {talkingPoints.map((tp) => (
                  <li key={tp.id} style={{ fontSize: 'var(--lf-text-sm)', marginBottom: 6, color: 'var(--lf-ink-2)' }}>
                    ...
                    {tp.isRequired && <> <Badge tone="wine">required</Badge></>}
                    {tp.description && <div style={{ fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-3)' }}>{tp.description}</div>}
                  </li>
                ))}
              </ul>
            )}
          ...
          <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
            <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-3)' }}>Qualification Questions</div>
            {qualifications.length === 0 ? (
              <p style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-3)' }}>No qualification questions defined.</p>
            ) : (
