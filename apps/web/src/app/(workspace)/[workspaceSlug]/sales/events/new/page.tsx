...
export default function NewEventPage() {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="lf-card" style={{ padding: 'var(--lf-space-5)', marginTop: 'var(--lf-space-4)', maxWidth: 640 }}>
        <div style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
          <div>
            <label className="lf-label" htmlFor="title">Title *</label>
            <input className="lf-input" id="title" name="title" required maxLength={200} />
          </div>
          <div>
            <label className="lf-label" htmlFor="description">Description</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--lf-space-4)' }}>
            <div>
              <label className="lf-label" htmlFor="eventType">Type</label>
            ...
            <div>
              <label className="lf-label" htmlFor="capacity">Capacity</label>
              <input className="lf-input" id="capacity" name="capacity" type="number" min={1} />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--lf-space-4)' }}>
            <div>
              <label className="lf-label" htmlFor="startAt">Start *</label>
              <input className="lf-input" id="startAt" name="startAt" type="datetime-local" required />
            </div>
            <div>
              <label className="lf-label" htmlFor="endAt">End *</label>
              <input className="lf-input" id="endAt" name="endAt" type="datetime-local" required />
            </div>
          </div>
          <div>
            <label className="lf-label" htmlFor="timezone">Timezone</label>
            <input className="lf-input" id="timezone" name="timezone" defaultValue="Asia/Dubai" />
          </div>
          <div>
            <label className="lf-label" htmlFor="location">Location</label>
            <input className="lf-input" id="location" name="location" maxLength={300} placeholder="Venue name or address" />
          </div>
          <div>
            <label className="lf-label" htmlFor="meetingUrl">Meeting URL</label>
            <input className="lf-input" id="meetingUrl" name="meetingUrl" type="url" placeholder="Google Meet, Zoom, etc." />
          </div>

          {error && <div style={{ color: 'var(--lf-vermillion)', fontSize: 'var(--lf-text-sm)' }}>{error}</div>}

          <div style={{ display: 'flex', gap: 'var(--lf-space-3)' }}>
            <button className="lf-btn" type="submit" disabled={saving}>{saving ? 'Creating…' : 'Create Event'}</button>
            <SalesLink href="/events" className="lf-btn lf-btn--ghost">Cancel</SalesLink>
          </div>
        </div>
      </form>
    </>
