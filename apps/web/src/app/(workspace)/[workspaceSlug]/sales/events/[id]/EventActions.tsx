    ...
    <section className="lf-card" style={{ padding: 'var(--lf-space-5)', marginBottom: 'var(--lf-space-4)' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--lf-space-3)', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 240px' }}>
          <label className="lf-label" htmlFor="template">
          ...
          <input
            ...
            id="template"
            value={template}
            placeholder="event_invite_v1"
            onChange={(event) => setTemplate(event.target.value)}
          />
        </div>
        <button
          className="lf-btn"
          disabled={!template.trim() || busy !== null}
          onClick={() => post('invite', { template: template.trim() }, 'invite')}
        >
          {busy === 'invite' ? 'Sending…' : 'Send on WhatsApp'}
        </button>
        <button className="lf-btn lf-btn--ghost" disabled={busy !== null} onClick={() => post('calls', {}, 'calls')}>
          {busy === 'calls' ? 'Queueing…' : 'Queue calls to me'}
        </button>
      </div>

      <p style={{ fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-3)', marginTop: 'var(--lf-space-3)' }}>
        WhatsApp Business only delivers pre-approved templates for business-initiated messages. The template receives
        the invitee name, event title, start time and location or meeting link, in that order.
      </p>
      ...
      {result && (
        <p
          style={{
            ...
            color: result.ok ? 'var(--lf-viridian)' : 'var(--lf-vermillion)',
          }}
        >
          {result.text}
        </p>
