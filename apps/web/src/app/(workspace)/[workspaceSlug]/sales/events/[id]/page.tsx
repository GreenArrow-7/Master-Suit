    ...
    <>
      ...
      >
        ...
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div
            className="lf-num"
            style={{ fontSize: 'var(--lf-text-2xl)', fontWeight: 700, color: 'var(--lf-viridian)' }}
          >
            {confirmed}
          </div>
        ...
        <div className="lf-card" style={{ padding: 'var(--lf-space-4)', textAlign: 'center' }}>
          <div
            className="lf-num"
            style={{ fontSize: 'var(--lf-text-2xl)', fontWeight: 700, color: 'var(--lf-vermillion)' }}
          >
            {declined}
          </div>
      ...
      {meetings.length > 0 && (
        <section className="lf-card" style={{ padding: 'var(--lf-space-5)', marginBottom: 'var(--lf-space-4)' }}>
          ...
          {meetings.map((meeting) => (
            ...
            >
              <div
                style={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--lf-space-3)', flexWrap: 'wrap' }}
              >
                <SalesLink href={`/calls/${meeting.id}`} style={{ fontWeight: 500 }}>
                  {meeting.recipientNumber ?? 'Meeting'} · {meeting.caller.fullName}
                </SalesLink>
                <Badge tone={meeting.status === 'COMPLETED' ? 'viridian' : 'slate'}>
      ...
      {event.description && (
        <section className="lf-card" style={{ padding: 'var(--lf-space-5)', marginBottom: 'var(--lf-space-4)' }}>
          ...
          {event.location && (
            <p style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-3)', marginTop: 'var(--lf-space-2)' }}>
              Location: {event.location}
            </p>
          )}
          {event.meetingUrl && (
            <p style={{ fontSize: 'var(--lf-text-sm)', marginTop: 'var(--lf-space-2)' }}>
              <SalesLink href={event.meetingUrl} target="_blank" rel="noopener noreferrer">
                Join meeting
              </SalesLink>
            </p>
          )}
      ...
      <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
        ...
        {event.invitees.length === 0 ? (
          <p style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-3)' }}>
            No invitees yet. Use the API to add invitees.
          </p>
        ) : (
                ...
                {event.invitees.map((inv) => (
                  <tr key={inv.id}>
                    ...
                    <td style={{ fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-3)' }}>
                      {inv.inviteChannel.toLowerCase()}
                    </td>
                    <td style={{ fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-3)' }}>
                      {inv.inviteSentAt
                        ? inv.inviteSentAt.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })
                        : 'not sent'}
                    </td>
                    <td>
                      <Badge tone={RSVP_TONE[inv.rsvpStatus]}>{inv.rsvpStatus.toLowerCase().replace(/_/g, ' ')}</Badge>
                    </td>
                    <td>
                      {can(ctx, 'events', 'EDIT') ? (
                        <RsvpControl
