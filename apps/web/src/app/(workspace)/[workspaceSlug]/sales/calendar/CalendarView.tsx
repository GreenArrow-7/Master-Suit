...
export default function CalendarView({ items }: { items: CalendarItem[] }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--lf-space-3)', marginBottom: 'var(--lf-space-4)' }}>
        <button className="lf-btn lf-btn--sm lf-btn--secondary" onClick={() => setOffset(o => o - 1)} aria-label="Previous month">&larr;</button>
        <h2 className="lf-h2" style={{ margin: 0, minWidth: 200, textAlign: 'center' }}>{monthLabel}</h2>
        <button className="lf-btn lf-btn--sm lf-btn--secondary" onClick={() => setOffset(o => o + 1)} aria-label="Next month">&rarr;</button>
        {offset !== 0 && (
          <button className="lf-btn lf-btn--sm lf-btn--secondary" onClick={() => setOffset(0)}>Today</button>
        )}
      </div>
              ...
              {day && (
                <>
                  <div style={{
                    ...
                    borderRadius: '50%', fontSize: 'var(--lf-text-sm)', fontWeight: isToday ? 700 : 400,
      ...
      {selectedDay != null && (
        <div className="lf-card" style={{ marginTop: 'var(--lf-space-4)' }}>
          <p className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-2)' }}>
            {new Date(year, month, Number(selectedDay)).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}
          </p>
          {selectedItems.length === 0 ? (
            <p style={{ color: 'var(--lf-ink-3)', fontSize: 'var(--lf-text-sm)' }}>Nothing scheduled</p>
          ) : (
            <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 'var(--lf-space-2)' }}>
              {selectedItems.map(item => (
                <li key={item.id} style={{
                  display: 'flex', alignItems: 'center', gap: 'var(--lf-space-2)',
                  fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink)',
                }}>
                  <span style={{
                    width: 8, height: 8, borderRadius: '50%', flexShrink: 0,
                    background: item.kind === 'activity'
                      ? 'var(--lf-wine-700, #7f1d4e)'
                      : item.completed
                        ? 'var(--lf-viridian, #16a34a)'
                        : 'var(--lf-brass, #ca8a04)',
                  }} />
                  <span style={{ textTransform: 'uppercase', fontSize: 'var(--lf-text-xs)', color: 'var(--lf-ink-3)', width: 60 }}>
                    {item.kind}
                  </span>
                  {item.label}
                </li>
