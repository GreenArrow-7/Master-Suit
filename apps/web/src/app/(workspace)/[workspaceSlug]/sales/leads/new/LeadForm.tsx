...
export default function LeadForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.detail ?? 'Could not create lead.'); return; }
  ...
  return (
    <form onSubmit={submit} style={{ display: 'grid', gap: 'var(--lf-space-4)' }} noValidate>
      ...
      <div className="lf-field">
        <label className="lf-label" htmlFor="fullName">Full name *</label>
        <input id="fullName" className="lf-input" value={fullName} onChange={(e) => setFullName(e.target.value)} required autoFocus />
      </div>
      ...
      <div className="lf-field">
        <label className="lf-label" htmlFor="company">Company</label>
        <input id="company" className="lf-input" value={company} onChange={(e) => setCompany(e.target.value)} />
      </div>

      <div className="lf-field">
        <label className="lf-label" htmlFor="jobTitle">Job title</label>
      ...
      <div style={{ display: 'flex', gap: 'var(--lf-space-3)', marginTop: 'var(--lf-space-2)' }}>
        <button className="lf-btn" type="submit" disabled={busy}>{busy ? 'Creating…' : 'Create lead'}</button>
        <SalesLink className="lf-btn lf-btn--secondary" href="/leads">Cancel</SalesLink>
      </div>
    </form>
  );
}
