    ...
    >
      ...
      {file && !report && (
        <>
          ...
          <div className="lf-table-wrap" style={{ maxHeight: 220, overflow: 'auto', marginBottom: 'var(--lf-space-3)' }}>
                    ...
                    <td>
                      <select
                        ...
                        onChange={(event) => {
                          ...
                          setMapping((current) =>
                            current.map((t, i) => (i === column ? target : LEAD_FIELDS.includes(t as never) && t === target ? 'notes+' : t)),
                          );
                        }}
                      >
                        {LEAD_FIELDS.map((f) => (
          ...
          {problems.length > 0 && (
            <ul style={{ ...small, margin: '0 0 var(--lf-space-2)', paddingLeft: 16, maxHeight: 90, overflowY: 'auto' }}>
          ...
          {ready.length > 0 && (
            <div className="lf-table-wrap" style={{ maxHeight: 180, overflow: 'auto', marginBottom: 'var(--lf-space-3)' }}>
              <table className="lf-table" style={{ minWidth: 0 }}>
                <thead>
                  <tr>
                    <th>Line</th>
                ...
                <tbody>
                  {ready.slice(0, 10).map((r) => (
                    <tr key={r.line}>
                      ...
                      {previewFields.map((f) => (
                        <td key={f} style={{ maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {r.values[f] ?? ''}
                        </td>
                      ))}
                    </tr>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button className="lf-btn lf-btn--sm" disabled={busy || toSend.length === 0} onClick={() => void importRows()}>
              {busy ? `Importing… ${progress}/${toSend.length}` : `Import ${toSend.length} lead${toSend.length === 1 ? '' : 's'}`}
            </button>
      ...
      {report && (
        <div style={{ marginTop: 'var(--lf-space-3)', fontSize: 'var(--lf-text-2xs)' }}>
          ...
          {report.failed.length > 0 && (
            <>
              <p style={{ margin: '6px 0 2px', color: 'var(--lf-vermillion)' }}>{report.failed.length} row(s) not imported:</p>
