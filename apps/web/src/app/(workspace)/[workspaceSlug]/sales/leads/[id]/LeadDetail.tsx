...
interface ActivityType { id: string; name: string; key: string }
interface TaskType { id: string; name: string; key: string }
interface User { id: string; fullName: string }
interface Stage { id: string; key: string; name: string }

interface Activity {
  id: string; outcome: string | null; notes: string | null;
  occurredAt: string; durationSecs: number | null;
  type: { name: string; key: string };
}

interface TaskItem {
  id: string; title: string; description: string | null;
  dueAt: string; priority: string; status: string;
...
interface LeadData {
  ...
  country: string | null;
  source: string; consentStatus: string; priority: string;
  slaState: string; score: number; grade: string | null;
  notes: string | null; tags: string[];
  nextFollowUpAt: string | null; lastActivityAt: string | null;
  createdAt: string;
...
export default function LeadDetail({ lead, stages, activityTypes, taskTypes, users, canEdit, canAssign, canDelete }: Props) {
  ...
  const handleAssign = (userId: string | null) => {
    setShowAssign(false);
    withBusy(async () => { await patchLead({ ownerId: userId }); })();
  };
  ...
  const handleStageChange = (stageKey: string) => {
    ...
    if (!target) return;
    withBusy(async () => { await patchLead({ stageId: target.id }); })();
  };

  return (
    <>
      {error && <div className="lf-alert" style={{ marginBottom: 'var(--lf-space-3)' }}>{error}</div>}
      ...
      <header style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--lf-space-4)', margin: 'var(--lf-space-3) 0 var(--lf-space-4)' }}>
        <span className="lf-avatar" style={{ width: 44, height: 44, fontSize: 'var(--lf-text-base)' }}>
          {lead.fullName.split(' ').slice(0, 2).map(p => p[0]).join('')}
        </span>
        <div style={{ minWidth: 0 }}>
          <h1 className="lf-h1" style={{ fontSize: 'var(--lf-text-2xl)' }}>{lead.fullName}</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--lf-space-3)', marginTop: 4, flexWrap: 'wrap' }}>
            <span className="lf-num" style={{ fontSize: 'var(--lf-text-xs)', color: 'var(--lf-ink-3)' }}>{lead.reference}</span>
            ...
            <span className="lf-score">
              <span className="lf-score__bar"><span className="lf-score__fill" style={{ width: `${lead.score}%` }} /></span>
              {lead.score}{lead.grade ? ` · ${lead.grade}` : ''}
            </span>
          </div>
        </div>

        <div style={{ marginLeft: 'auto', display: 'flex', gap: 'var(--lf-space-2)', flexWrap: 'wrap', position: 'relative' }}>
          {/* Call */}
          <button
      ...
      <nav className="lf-tabs" style={{ margin: 'var(--lf-space-5) 0 var(--lf-space-4)' }} role="tablist">
        {TABS.map(t => (
          ...
          >
            ...
            {t === 'Timeline' && lead.activities.length > 0 && <span className="lf-tab__count">{lead.activities.length}</span>}
          </button>
        ))}
      </nav>

      {/* Tab content */}
      {tab === 'Overview' && <OverviewTab lead={lead} editing={editing} setEditing={setEditing} patchLead={patchLead} />}
  ...
  return (
    <>
      ...
      <div style={{
        ...
        borderRadius: 6, padding: 'var(--lf-space-1)', minWidth: 180,
        boxShadow: '0 4px 12px rgba(0,0,0,.12)', maxHeight: 280, overflowY: 'auto',
...
function OverviewTab({ lead, editing, setEditing, patchLead }: {
  lead: Props['lead']; editing: boolean; setEditing: (v: boolean) => void;
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 320px) minmax(0, 1fr)', gap: 'var(--lf-space-4)', alignItems: 'start' }}>
      <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
        ...
        {err && <div className="lf-alert" style={{ marginBottom: 'var(--lf-space-3)', fontSize: 'var(--lf-text-sm)' }}>{err}</div>}
          ...
          {lead.tags.length > 0 && (
            <div>
              ...
              <dd style={{ margin: '2px 0 0', display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                {lead.tags.map(t => <Badge key={t} tone="slate">{t}</Badge>)}
              </dd>
            </div>
          )}
        </dl>
        {editing && (
          <div style={{ display: 'flex', gap: 'var(--lf-space-2)', marginTop: 'var(--lf-space-4)' }}>
            ...
            <button className="lf-btn lf-btn--secondary lf-btn--sm" onClick={() => setEditing(false)}>Cancel</button>
          </div>
        )}
      </section>
...
function SummaryCard({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="lf-card" style={{ padding: 'var(--lf-space-4)' }}>
      ...
      <div className="lf-num" style={{ fontSize: 'var(--lf-text-xl)', marginTop: 4, color }}>{value}</div>
    </div>
  );
}
...
function TimelineTab({ lead, activityTypes, router }: {
  lead: Props['lead']; activityTypes: ActivityType[];
  ...
  return (
    <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--lf-space-4)' }}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ marginBottom: 'var(--lf-space-4)', display: 'grid', gap: 'var(--lf-space-3)', padding: 'var(--lf-space-4)', border: '1px solid var(--lf-line)', borderRadius: 6 }}>
          {err && <div className="lf-alert" style={{ fontSize: 'var(--lf-text-sm)' }}>{err}</div>}
          <div className="lf-field">
            <label className="lf-label">Type</label>
            <select className="lf-input" value={form.typeId} onChange={e => setForm(f => ({ ...f, typeId: e.target.value }))}>
              {activityTypes.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
          </div>
          <div className="lf-field">
            <label className="lf-label">Outcome</label>
            <input className="lf-input" value={form.outcome} onChange={e => setForm(f => ({ ...f, outcome: e.target.value }))} placeholder="e.g. Interested, No answer..." />
          </div>
          <div className="lf-field">
            <label className="lf-label">Notes</label>
            <textarea className="lf-input" rows={3} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
          </div>
          <div className="lf-field">
            <label className="lf-label">Duration (minutes)</label>
            <input className="lf-input" type="number" min="0" value={form.durationMins} onChange={e => setForm(f => ({ ...f, durationMins: e.target.value }))} />
          </div>
          <button className="lf-btn lf-btn--sm" type="submit" disabled={saving}>{saving ? 'Saving...' : 'Log'}</button>
        </form>
      )}
      ...
      {lead.activities.length === 0 ? (
        ...
        <div className="lf-timeline">
          {lead.activities.map(a => (
            <div key={a.id} className="lf-timeline__item" data-kind={a.type.key.startsWith('call') ? 'call' : 'default'}>
              ...
              {a.outcome && <div style={{ fontSize: 'var(--lf-text-sm)', color: 'var(--lf-ink-2)', marginTop: 2 }}>{a.outcome}</div>}
              {a.notes && <div style={{ fontSize: 'var(--lf-text-xs)', color: 'var(--lf-ink-3)', marginTop: 2 }}>{a.notes}</div>}
            </div>
          ))}
        </div>
      )}
...
function TasksTab({ lead, taskTypes, router }: {
  lead: Props['lead']; taskTypes: TaskType[];
  router: ReturnType<typeof useRouter>;
}) {
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ typeId: taskTypes[0]?.id ?? '', title: '', dueAt: '', priority: 'MEDIUM', description: '' });
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState<string | null>(null);
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ marginBottom: 'var(--lf-space-4)', display: 'grid', gap: 'var(--lf-space-3)', padding: 'var(--lf-space-4)', border: '1px solid var(--lf-line)', borderRadius: 6 }}>
          ...
          <div className="lf-field">
            ...
            <input className="lf-input" required value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--lf-space-3)' }}>
            ...
            <div className="lf-field">
              ...
              <select className="lf-input" value={form.priority} onChange={e => setForm(f => ({ ...f, priority: e.target.value }))}>
          ...
          <div className="lf-field">
            ...
            <input className="lf-input" type="date" required value={form.dueAt} onChange={e => setForm(f => ({ ...f, dueAt: e.target.value }))} />
          </div>
          <div className="lf-field">
            <label className="lf-label">Description</label>
            <textarea className="lf-input" rows={2} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
          </div>
          <button className="lf-btn lf-btn--sm" type="submit" disabled={saving}>{saving ? 'Saving...' : 'Create'}</button>
        </form>
      )}
      ...
      {lead.tasks.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 'var(--lf-space-3)' }}>
          {lead.tasks.map(t => {
            ...
            return (
              <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--lf-space-3)', padding: 'var(--lf-space-3)', border: '1px solid var(--lf-line)', borderRadius: 6 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 500, fontSize: 'var(--lf-text-sm)' }}>{t.title}</div>
                  <div style={{ display: 'flex', gap: 'var(--lf-space-2)', marginTop: 4, alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 'var(--lf-text-xs)', color: 'var(--lf-ink-3)' }}>{t.type.name}</span>
                    <Badge value={t.priority} />
                    <Badge value={t.status} />
                    <span style={{ fontSize: 'var(--lf-text-xs)', color: overdue ? 'var(--lf-vermillion)' : 'var(--lf-ink-3)', fontWeight: overdue ? 600 : 400 }}>
                      Due {fmtDate(t.dueAt)}
                    </span>
                  </div>
                </div>
...
function NotesTab({ lead, patchLead, canEdit }: { lead: Props['lead']; patchLead: (d: Record<string, unknown>) => Promise<void>; canEdit: boolean }) {
  ...
  return (
    <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
      <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-4)' }}>Notes</div>
      <textarea
        className="lf-input"
        rows={10}
      ...
      />
      {canEdit && (
        <div style={{ display: 'flex', gap: 'var(--lf-space-2)', marginTop: 'var(--lf-space-3)', alignItems: 'center' }}>
          <button className="lf-btn lf-btn--sm" disabled={saving} onClick={handleSave}>
...
function DocumentsTab({ documents }: { documents: Doc[] }) {
  if (documents.length === 0) {
    return (
      <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
        <div className="lf-eyebrow" style={{ marginBottom: 'var(--lf-space-4)' }}>Documents</div>
        <p style={{ color: 'var(--lf-ink-3)', fontSize: 'var(--lf-text-sm)', margin: 0 }}>No documents attached to this lead.</p>
      </section>
    );
  }
  ...
  return (
    <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
      ...
      <div style={{ display: 'grid', gap: 'var(--lf-space-3)' }}>
        {documents.map(d => (
          <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--lf-space-3)', padding: 'var(--lf-space-3)', border: '1px solid var(--lf-line)', borderRadius: 6 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
