...
const SYSTEM_VIEWS: SystemView[] = [
  ...
  { key: 'yesterday', name: "Yesterday's Leads", icon: '◇', filter: { field: 'createdAt', cmp: 'relative', value: 'yesterday' } },
  { key: 'this-week', name: 'This Week', icon: '◇', filter: { field: 'createdAt', cmp: 'relative', value: 'this_week' } },
  { key: 'this-month', name: 'This Month', icon: '◇', filter: { field: 'createdAt', cmp: 'relative', value: 'this_month' } },
  { key: 'overdue', name: 'Overdue Follow-up', icon: '!', filter: { field: 'nextFollowUpAt', cmp: 'relative', value: 'overdue' } },
  ...
  { key: 'cold', name: 'No Activity (30d)', icon: '○', filter: { field: 'lastActivityAt', cmp: 'relative', value: 'last_30_days' } },
];

export default async function SmartViewsPage({ searchParams }: { searchParams: Promise<{ view?: string; id?: string; q?: string }> }) {
  ...
  const [rows, stages, counts, users, taskTypes, setting] = await Promise.all([
    ...
    prisma.user.findMany({ where: { tenantId: ctx.tenantId, status: 'ACTIVE', deletedAt: null }, orderBy: { fullName: 'asc' }, select: { id: true, fullName: true } }),
    prisma.taskType.findMany({ where: { tenantId: ctx.tenantId, isActive: true }, orderBy: { name: 'asc' }, select: { id: true, name: true } }),
