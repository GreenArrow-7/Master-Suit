...
export default function ViewSidebar({ systemViews, savedViews, activeSystemKey, activeSavedId }: Props) {
  return (
    <aside style={{
      width: 220, flex: 'none', borderRight: '1px solid var(--lf-line)',
      paddingRight: 'var(--lf-space-4)', overflowY: 'auto',
    }}>
      <div className="lf-eyebrow" style={{ padding: '0 0 6px', color: 'var(--lf-ink-3)' }}>System Views</div>
            ...
            <li key={v.key}>
              <a
                ...
                style={{
                  ...
                  padding: '5px 8px', borderRadius: 'var(--lf-radius-sm)',
                  fontSize: 'var(--lf-text-sm)', textDecoration: 'none',
      ...
      {savedViews.length > 0 && (
        <>
          <div className="lf-eyebrow" style={{ padding: 'var(--lf-space-4) 0 6px', color: 'var(--lf-ink-3)' }}>Saved Views</div>
          <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
            {savedViews.map((v) => {
              ...
              return (
                <li key={v.id}>
                  ...
                  >
                    ...
                    {v.name}
                    {!v.mine && <span style={{ marginLeft: 'auto', fontSize: 'var(--lf-text-2xs)', color: 'var(--lf-ink-4)' }}>shared</span>}
                  </a>
                </li>
              );
            })}
