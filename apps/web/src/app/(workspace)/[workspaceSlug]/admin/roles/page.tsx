  ...
  return <div style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
    <section>
      ...
      <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)', maxWidth: '80ch' }}>
        A role is a set of permissions, each with a scope: how much of the workspace it reaches.
        You can only work on roles below your own level, and you can never grant more than you hold yourself —
        otherwise this screen would be a route to promoting yourself.
      </p>
    </section>
    ...
    <section>
      ...
      <WorkspaceTable
        ...
        rows={roles.map((role) => [
          <span key="n">{role.name}{role.isSystem ? <span className="lf-badge" style={{ marginLeft: 6 }}>system</span> : null}</span>,
          <code key="k" style={{ fontSize: 'var(--lf-text-xs)' }}>{role.key}</code>,
          role.rank,
          role.defaultScope.toLowerCase(),
          role.users,
          role.permissions,
          <span key="a" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <Link className="lf-btn lf-btn--ghost" href={`?roleId=${role.id}`}>Permissions</Link>
            {mayManage && role.editable && !role.isSystem && role.users === 0 && <WorkspaceActionButton
              endpoint={`${endpoint}/delete`} body={{ roleId: role.id }} label="Delete" variant="danger"
    ...
    {selected && <section style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
    ...
    {mayManage && <section>
      ...
      <p style={{ margin: '0 0 10px', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
        Rank orders seniority: lower is more senior. You cannot create a role at or above your own rank ({ctx.actor.roleRank}).
      </p>
      <WorkspaceRecordForm
        ...
        fields={[
          ...
          { name: 'defaultScope', label: 'Default scope', type: 'select', options: ['OWN', 'TEAM', 'BRANCH', 'REGION', 'ORGANIZATION'].map((value) => ({ value, label: value.toLowerCase() })) },
    ...
    {mayManage && <section>
      ...
      <p style={{ margin: '0 0 10px', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
        An additional, time-boxed grant alongside whatever the account already holds — how cover works while someone is on leave.
        Leave the dates blank for an open-ended grant; a grant with an end date lapses on its own rather than depending on anyone remembering.
      </p>
      <WorkspaceRecordForm
        endpoint={`${endpoint}/assign`}
        submitLabel="Assign role"
        fields={[
          { name: 'membershipId', label: 'Account', type: 'select', required: true, options: memberships.map((membership) => ({ value: membership.id, label: `${membership.platformUser.fullName} · ${membership.platformUser.email}` })) },
          { name: 'roleId', label: 'Role', type: 'select', required: true, options: roles.filter((role) => role.editable).map((role) => ({ value: role.id, label: `${role.name} (rank ${role.rank})` })) },
    ...
    <section>
      ...
      <p style={{ margin: '0 0 10px', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
        Every grant and revocation is kept, including expired ones — who held what, and when, is the record that matters after the fact.
      </p>
      <WorkspaceTable
        headers={['Account', 'Role', 'Window', 'State', '']}
        empty="No additional role assignments."
        rows={history.map((assignment) => [
          <span key="h">{assignment.holder}<div style={{ color: 'var(--lf-ink-500)', fontSize: 'var(--lf-text-xs)' }}>{assignment.email}</div></span>,
          ...
          <span className="lf-badge" key="s">{assignment.inForce ? 'in force' : assignment.status.toLowerCase()}</span>,
          mayManage && assignment.status === 'ACTIVE'
            ? <WorkspaceActionButton key="r" endpoint={`${endpoint}/revoke`} body={{ assignmentId: assignment.id }} label="Revoke" variant="ghost" promptFor={{ name: 'reason', label: 'Reason', required: false }} />
