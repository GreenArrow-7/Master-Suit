...
export default async function Page({ params }: { params: Promise<{ workspaceSlug: string }> }) {
  ...
  const scope = hr
    ? {}
    : approver
      ? { OR: [{ employeeId: self?.id ?? '' }, { approverId: self?.id ?? '' }] }
      : { employeeId: self?.id ?? '' };
        ...
        <section>
          ...
          >
            {balances.map((balance) => (
              <article className="lf-card" key={balance.leaveTypeId} style={{ padding: 'var(--lf-space-5)' }}>
                <div className="lf-eyebrow">
                  {balance.name}
                  {balance.paid ? '' : ' · unpaid'}
                </div>
                <div style={{ fontFamily: 'var(--lf-font-display)', fontSize: 32, marginTop: 6 }}>
                  {balance.availableDays}
                </div>
                <div style={{ color: 'var(--lf-ink-500)', fontSize: 'var(--lf-text-xs)' }}>
                  days available · {balance.accruedDays} accrued
                  {balance.carriedForwardDays ? ` · ${balance.carriedForwardDays} carried` : ''} · {balance.takenDays}{' '}
                  used
                </div>
              </article>
            ))}
            ...
            rows={pending.map((request) => [
              ...
              <span style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }} key="decide">
                <WorkspaceActionButton
                  endpoint={`${actions}/leave-approve`}
                  body={{ requestId: request.id }}
                  label="Approve"
                />
                <WorkspaceActionButton
                  endpoint={`${actions}/leave-reject`}
                  body={{ requestId: request.id }}
                  label="Reject"
                  variant="danger"
          ...
          fields={[
            ...(hr
              ? [
                  {
                    name: 'employeeId',
                    label: 'Employee (HR only — leave blank for yourself)',
              ...
              : []),
            {
              ...
              type: 'select',
              required: true,
              options: types.map((type) => ({ value: type.id, label: `${type.name}${type.paid ? '' : ' (unpaid)'}` })),
            },
      ...
      <section>
        ...
        <WorkspaceTable
          ...
          rows={requests.map((request) => [
            ...
            (hr || request.employeeId === self?.id) ? (
              <WorkspaceActionButton
                key="cancel"
                endpoint={`${actions}/leave-cancel`}
                body={{ requestId: request.id }}
                label="Cancel"
                variant="ghost"
        ...
        <section className="lf-card" style={{ padding: 'var(--lf-space-5)', display: 'grid', gap: 10 }}>
          <div>
            ...
            <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
              Rolls unused accruing leave into the next year, up to each type&apos;s cap. Run once, after the year
              closes.
            </p>
          </div>
          <WorkspaceActionButton
