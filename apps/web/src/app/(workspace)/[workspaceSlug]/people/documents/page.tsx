...
export default async function Page({ params }: { params: Promise<{ workspaceSlug: string }> }) {
  ...
  const { ctx } = await resolveWorkspacePage(workspaceSlug, { module: 'HRMS', permission: ['hr_documents', 'VIEW'] });
        ...
        <WorkspaceTable
          ...
          rows={documents.map((document) => {
            const daysLeft = document.expiresAt
              ? Math.round((document.expiresAt.getTime() - Date.now()) / 86_400_000)
              : null;
            return [
              ...
              document.expiresAt ? (
                <span key="e">
                  {document.expiresAt.toLocaleDateString('en-AE', { timeZone: 'UTC' })}
                  <span className="lf-badge" style={{ marginLeft: 6 }}>
                    {expirySeverity(daysLeft!)}
                  </span>
                </span>
              ) : (
                '—'
              ...
              document.storageKey ? (
                <a key="d" className="lf-btn lf-btn--ghost" href={`${base}/documents/${document.id}/download`}>
                  Download
                </a>
              ) : (
              ...
              hr ? (
                <WorkspaceActionButton
                  key="x"
                  endpoint={`${base}/actions/document-delete`}
