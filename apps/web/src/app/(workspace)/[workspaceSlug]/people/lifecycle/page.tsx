    ...
    return (
      <section className="lf-card" style={{ padding: 'var(--lf-space-6)' }}>
        ...
        <p style={{ color: 'var(--lf-ink-600)' }}>
          Onboarding, offboarding and final settlement are restricted to HR and administrators.
        </p>
      </section>
    );
  }
    ...
    <div style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
      <section>
        ...
        <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)' }}>
          Blocking checklist steps gate activation and exit. An unstamped visa means someone cannot legally work; open
          clearance means nobody walks out with a laptop and a live visa.
        </p>
      </section>
      ...
      <section>
        ...
        <WorkspaceTable
          ...
          rows={dashboard.joining.map((row) => [
            ...
            row.openBlockers === 0 ? (
              <span className="lf-badge" key="b">
                clear
              </span>
            ) : (
              <strong key="b">{row.openBlockers}</strong>
            ),
            <span style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }} key="a">
              <Link className="lf-btn lf-btn--ghost" href={`?employeeId=${row.employeeId}`}>
                Checklist
              </Link>
              <WorkspaceActionButton
                endpoint={`${actions}/employee-activate`}
                body={{ employeeId: row.employeeId }}
                label="Activate"
              />
            </span>,
          ])}
        ...
        >
          <div>
            <div className="lf-eyebrow">
              {selected.employeeNumber} · {selected.employmentStatus}
            </div>
            <h2 style={{ fontSize: 'var(--lf-text-lg)', margin: '6px 0 0' }}>
              {selected.membership.platformUser.fullName}
            </h2>
          </div>
          ...
          <WorkspaceTable
            ...
            rows={checklist.map((task) => [
              ...
              task.completed ? (
                <WorkspaceActionButton
                  key="x"
                  endpoint={`${actions}/checklist-reopen`}
                  body={{ taskId: task.id }}
                  label="Reopen"
                  variant="ghost"
                />
                ...
                <WorkspaceActionButton
                  key="x"
                  endpoint={`${actions}/checklist-complete`}
                  body={{ taskId: task.id }}
                  label="Complete"
                />
              ),
          ...
          {settlement && (
            <div>
              ...
              <WorkspaceTable
                ...
                rows={[
                  [
                    `Gratuity — ${settlement.gratuityDays} days over ${settlement.serviceYears} years${settlement.gratuityCapped ? ' (capped at two years’ basic)' : ''}`,
                    'Basic salary',
                    money(settlement.gratuityAmount),
                  ],
                  [
                    `Leave encashment — ${settlement.unusedLeaveDays} unused days`,
                    'Basic salary',
                    money(settlement.leaveEncashmentAmount),
                  ],
                  [
                    `Notice shortfall — ${settlement.noticeShortfallDays} days`,
                    settlement.noticePayBasis === 'total' ? 'Total remuneration' : 'Basic salary',
                    `− ${money(settlement.noticeDeductionAmount)}`,
                  ],
                  [
                    <strong key="t">Total payable</strong>,
                    '',
                    <strong key="v">{money(settlement.totalPayable)}</strong>,
                  ],
                ]}
              />
              <p style={{ margin: '8px 0 0', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-xs)' }}>
                {settlement.note}
                {settlement.clearanceComplete
                  ? ''
                  : ` Outstanding clearance: ${settlement.outstandingClearances.join(', ')}.`}
              </p>
            </div>
          )}
