  ...
  return <div style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
    <section>
      ...
      <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)' }}>
        Employees record their own attendance by face at <Link href={`/${workspaceSlug}/people/check-in`}>check-in</Link>. Manual entry below is for HR corrections.
      </p>
    </section>
    ...
    {hr && <section>
      ...
      <p style={{ margin: '0 0 10px', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
        Simulated locations and repeated face or liveness failures. A rejected attempt is the record of who tried — the capture frame is kept encrypted against the punch.
      </p>
      <WorkspaceTable
    ...
    <section>
      ...
      <WorkspaceRecordForm
        ...
        fields={[
          { name: 'employeeId', label: 'Employee', type: 'select', required: true, options: employees.map((employee) => ({ value: employee.id, label: employee.membership.platformUser.fullName })) },
          { name: 'workDate', label: 'Work date', type: 'date', required: true },
          { name: 'status', label: 'Status', type: 'select', required: true, options: ['PRESENT', 'ABSENT', 'LATE', 'REMOTE', 'ON_LEAVE'].map((value) => ({ value, label: value.replace('_', ' ') })) },
    ...
    <section>
      ...
      <WorkspaceTable
        ...
        rows={records.map((record) => [
          ...
          record.workMinutes ? `${Math.floor(record.workMinutes / 60)}h ${String(record.workMinutes % 60).padStart(2, '0')}m` : '—',
          record.location?.name ?? '—',
          <span className="lf-badge" key="s">{record.status}</span>,
        ])}
      />
    </section>
  </div>;
