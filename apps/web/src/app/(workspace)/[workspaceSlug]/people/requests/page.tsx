...
export default async function Page({ params }: { params: Promise<{ workspaceSlug: string }> }) {
  ...
  if (!self) {
    return <section className="lf-card" style={{ padding: 'var(--lf-space-6)' }}>
      ...
      <p style={{ color: 'var(--lf-ink-600)' }}>Your account has no employee record in this workspace, so there is nothing to raise a request against.</p>
    </section>;
  }
  ...
  return <div style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
    <section>
      ...
      <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)', maxWidth: '80ch' }}>
        ...
        an agent at a client site, or someone whose phone died at the door, needs a path that ends in a human decision.
        Neither route weakens the fence: an approved temporary site becomes a real, date-bounded location, and an approved
        exception writes a corrected punch while leaving the original refusal in the log as evidence.
      </p>
    </section>
    ...
    <section>
      ...
      <WorkspaceRecordForm
        ...
        fields={[
          { name: 'requestedAction', label: 'What was missed', type: 'select', required: true, options: [{ value: 'CHECK_IN', label: 'Check in' }, { value: 'CHECK_OUT', label: 'Check out' }] },
          { name: 'requestedFor', label: 'When it should have been recorded', type: 'datetime-local', required: true },
          { name: 'reasonCode', label: 'Reason', type: 'select', required: true, options: EXCEPTION_REASONS.map((reason) => ({ value: reason, label: reason.replace(/_/g, ' ') })) },
      ...
      <WorkspaceTable
        ...
        rows={exceptions.map((request) => {
          ...
          return [
            ...
            <span key="r">{request.reasonCode.replace(/_/g, ' ')}{request.reasonText ? <div style={{ color: 'var(--lf-ink-500)', fontSize: 'var(--lf-text-xs)' }}>{request.reasonText}</div> : null}</span>,
            request.nearestLocation?.name
              ? `${request.nearestLocation.name}${request.distanceM == null ? '' : ` · ${Math.round(request.distanceM)} m`}`
              : '—',
            <span className="lf-badge" key="s">{request.status.replace(/_/g, ' ').toLowerCase()}</span>,
            mayDecide
              ? <span style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }} key="d">
                  <WorkspaceActionButton endpoint={`${actions}/exception-decide`} body={{ requestId: request.id, approve: true }} label={atManagerStage ? 'Endorse' : 'Approve'} promptFor={{ name: 'comment', label: 'Comment' }} />
                  <WorkspaceActionButton endpoint={`${actions}/exception-decide`} body={{ requestId: request.id, approve: false }} label="Reject" variant="danger" promptFor={{ name: 'comment', label: 'Reason for rejection' }} />
                </span>
              : <span key="d" style={{ color: 'var(--lf-ink-500)', fontSize: 'var(--lf-text-xs)' }}>{request.hrComment ?? request.managerComment ?? '—'}</span>,
          ];
        })}
      />
    </section>
    ...
    <section>
      ...
      <p style={{ margin: '0 0 10px', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
        ...
        On approval it becomes a real work location with hard start and end dates, and the named staff are assigned to it for that window only.
      </p>
      <WorkspaceRecordForm
        ...
        fields={[
          ...
          { name: 'employeeIds', label: 'Employee covered', type: 'select', required: true, options: employees.map((employee) => ({ value: employee.id, label: `${employee.employeeNumber} · ${employee.membership.platformUser.fullName}` })) },
    ...
    <section>
      ...
      <WorkspaceTable
        ...
        rows={temporary.map((request) => [
          <span key="n">{request.name}<div style={{ color: 'var(--lf-ink-500)', fontSize: 'var(--lf-text-xs)' }}>{request.reason}</div></span>,
          ...
          <span className="lf-badge" key="s">{request.status.toLowerCase()}</span>,
          // The requester never decides their own, however senior they are.
          hr && request.status === 'PENDING' && request.requestedById !== self.id
            ? <span style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }} key="d">
                <WorkspaceActionButton endpoint={`${actions}/temporary-decide`} body={{ requestId: request.id, approve: true }} label="Approve" confirm="Approving creates a live work location and assigns the named staff to it for the requested window." />
                <WorkspaceActionButton endpoint={`${actions}/temporary-decide`} body={{ requestId: request.id, approve: false }} label="Reject" variant="danger" promptFor={{ name: 'note', label: 'Reason', required: false }} />
              </span>
