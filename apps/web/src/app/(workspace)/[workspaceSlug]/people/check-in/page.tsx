    ...
    <div style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
      <section>
        ...
        <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)' }}>
          The camera frame is verified on the server, inside your assigned work location. Face verification is the only
          way to record attendance — there is no PIN fallback.
        </p>
      </section>
      ...
      {!status.engineReady && (
        <section className="lf-alert" role="alert">
          ...
          <div style={{ marginTop: 6, whiteSpace: 'pre-wrap', fontSize: 'var(--lf-text-sm)' }}>
            {status.engineDetail}
          </div>
          <div style={{ marginTop: 6, fontSize: 'var(--lf-text-sm)' }}>
            Attendance fails closed on purpose: nothing is recorded while the engine is down.
          </div>
        </section>
      )}
        ...
        {(
          [
            ...
            [
              'Face enrolment',
              status.enrolled
                ? `${status.enrolledSamples} samples`
                : `${status.enrolledSamples} of ${status.samplesRequired}`,
            ],
      ...
      {!status.consentGrantedAt ? (
        <section className="lf-card" style={{ padding: 'var(--lf-space-5)', display: 'grid', gap: 10 }}>
          <div>
            ...
            <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
              Face check-in stores a 512-number mathematical template of your face, not a photograph of it, and the
              capture frame from each punch is kept encrypted as evidence for a limited period. You can withdraw consent
              at any time; withdrawal deletes your templates immediately. Without consent no biometric data is processed
              at all.
            </p>
          </div>
        ...
        <section style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
          {status.enrolled ? (
            ...
            <div className="lf-alert">
              Your face is not enrolled yet ({status.enrolledSamples} of {status.samplesRequired} samples). HR does this
              in person.
            </div>
          )}
          <div>
      ...
      {hr && (
        <section className="lf-card" style={{ padding: 'var(--lf-space-5)', display: 'grid', gap: 12 }}>
          <div>
            ...
            <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>
              Done in person, with the employee present. Their biometric consent must be recorded first. Raw images are
              never stored — only the template.
            </p>
          </div>
              ...
              <select className="lf-input" name="enrol" defaultValue={enrol ?? ''}>
                ...
                {employees.map((row) => (
                  <option key={row.id} value={row.id}>
                    {row.employeeNumber} · {row.membership.platformUser.fullName}
                  </option>
                ))}
              </select>
            </label>
      ...
      <section>
        ...
        <WorkspaceTable
          ...
          rows={days.map((day) => [
            ...
            day.workMinutes
              ? `${Math.floor(day.workMinutes / 60)}h ${String(day.workMinutes % 60).padStart(2, '0')}m`
              : '—',
            day.location?.name ?? '—',
            <span className="lf-badge" key="s">
              {day.checkOutAt ? 'complete' : 'open'}
            </span>,
          ])}
        />
      </section>
