  ...
  return <div style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
    <section>
      ...
      <p style={{ margin: '6px 0 0', color: 'var(--lf-ink-600)' }}>
        The geofence attendance is measured against. Coordinates never reach the browser during a punch — the distance is computed on the server, because a client-reported &ldquo;I am inside&rdquo; is exactly what an attacker would forge.
        A location left in <strong>DRAFT</strong> is not a candidate for any check-in.
      </p>
    </section>
    ...
    <section>
      ...
      <WorkspaceRecordForm
        ...
        fields={[
          ...
          { name: 'latitude', label: 'Latitude (stand in the office and read your phone GPS)', type: 'number', required: true },
          ...
          { name: 'maxAccuracyMeters', label: 'Worst acceptable GPS accuracy (metres)', type: 'number', placeholder: '100' },
          { name: 'openingTime', label: 'Opening time', type: 'time' },
          { name: 'closingTime', label: 'Closing time', type: 'time' },
          { name: 'emirate', label: 'Emirate' },
          { name: 'status', label: 'Status', type: 'select', options: [{ value: 'ACTIVE', label: 'Active' }, { value: 'DRAFT', label: 'Draft' }, { value: 'RETIRED', label: 'Retired' }] },
        ]}
      />
    </section>
    ...
    <section>
      ...
      <WorkspaceTable
        ...
        rows={locations.map((location) => [
          ...
          <span className="lf-badge" key="s">{location.status}</span>,
        ])}
      />
    </section>
    ...
    {hr && <section>
      ...
      <WorkspaceRecordForm
        ...
        submitLabel="Assign"
        fields={[
          { name: 'employeeId', label: 'Employee', type: 'select', required: true, options: employees.map((employee) => ({ value: employee.id, label: `${employee.employeeNumber} · ${employee.membership.platformUser.fullName}` })) },
          { name: 'locationId', label: 'Location', type: 'select', required: true, options: locations.map((location) => ({ value: location.id, label: `${location.name} (${location.status})` })) },
          { name: 'assignmentType', label: 'Type', type: 'select', options: ['PRIMARY', 'SECONDARY', 'TEMPORARY'].map((value) => ({ value, label: value.toLowerCase() })) },
          { name: 'effectiveFrom', label: 'Effective from', type: 'date' },
          { name: 'effectiveTo', label: 'Effective to', type: 'date' },
          { name: 'checkoutRule', label: 'Check-out rule', type: 'select', options: [
    ...
    <section>
      ...
      <WorkspaceTable
        ...
        rows={assignments.map((assignment) => [
          ...
          <span className="lf-badge" key="s">{assignment.status}</span>,
