 ...
 */
export default function DocumentUpload({ endpoint, employees, kinds }: {
  endpoint: string;
  ...
  return <form onSubmit={submit} className="lf-form-section">
    ...
    <div className="lf-form-grid">
      <label className="lf-field"><span className="lf-label">Employee</span>
      ...
      <label className="lf-field"><span className="lf-label">Document kind</span>
        <select className="lf-input" name="kind" required>
          <option value="">Select…</option>
          {kinds.map((kind) => <option key={kind} value={kind}>{kind.replace(/_/g, ' ')}</option>)}
        </select>
      </label>
      <label className="lf-field"><span className="lf-label">Document number</span>
        <input className="lf-input" name="number" /></label>
      <label className="lf-field"><span className="lf-label">Issued on</span>
        <input className="lf-input" name="issuedAt" type="date" /></label>
      <label className="lf-field"><span className="lf-label">Expires on</span>
        <input className="lf-input" name="expiresAt" type="date" /></label>
      <label className="lf-field"><span className="lf-label">File (PDF or image)</span>
        <input className="lf-input" name="file" type="file" accept=".pdf,image/jpeg,image/png,image/webp,image/heic" required /></label>
    </div>
    <div><button className="lf-btn" type="submit" disabled={busy}>{busy ? 'Uploading…' : 'Upload document'}</button></div>
  </form>;
}
