...
export default function HrPolicyForm({
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      setSaved(
        data.changed?.length
          ? `Saved ${data.changed.length} change${data.changed.length === 1 ? '' : 's'}.`
          : 'No changes to save.',
      );
  ...
  return (
    <form onSubmit={save} style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
      {error && (
            ...
            return (
              ...
              >
                <label
                  ...
                  style={{ fontWeight: 600, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}
                >
                  {definition.label}
                  {definition.statutory && (
                    <span
                      className="lf-badge"
                      title="This figure has legal weight. Changing it below the statutory minimum is not lawful in the UAE."
                    >
                      statutory
                    </span>
                  )}
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  {definition.type === 'number' && (
                    <>
                      <input
                        ...
                        onChange={(event) =>
                          set(definition.key, event.target.value === '' ? '' : Number(event.target.value))
                        }
                        style={{ maxWidth: 160 }}
                      />
                      {definition.unit && (
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                      {DAY_NAMES.map((name, day) => {
                        ...
                        return (
                          <label
                            key={name}
                            style={{ display: 'flex', gap: 5, alignItems: 'center', fontSize: 'var(--lf-text-sm)' }}
                          >
                            <input
                              type="checkbox"
                              disabled={readOnly}
                  ...
                  {!readOnly && !isDefault && (
                    <button
                      ...
                      type="button"
                      onClick={() => set(definition.key, definition.default)}
                    >
