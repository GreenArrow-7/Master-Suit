...
type IconName = 'home' | 'bell' | 'task' | 'lead' | 'deal' | 'company' | 'contact' | 'activity' | 'calendar' | 'call' | 'campaign' | 'report' | 'people' | 'attendance' | 'leave' | 'org' | 'document' | 'settings' | 'shield';

export default function WorkspaceSidebar({ slug, name, modules, permitted, workspaces, user }: {
  slug: string;
  name: string;
  modules: string[];
  ...
  useEffect(() => {
    ...
    setMobileOpen(false);
  }, [activeModule, pathname, slug]);

  const sections = useMemo<Section[]>(() => {
    const common: Section = { label: 'Work', items: [
    ...
    if (activeModule === 'people') return [common, { label: 'People', items: [
      { label: 'People overview', href: `/${slug}/people`, icon: 'people' },
      { label: 'Employees', href: `/${slug}/people/employees`, icon: 'people' },
      { label: 'Check in', href: `/${slug}/people/check-in`, icon: 'attendance' },
      { label: 'Attendance', href: `/${slug}/people/attendance`, icon: 'attendance' },
      { label: 'Leave', href: `/${slug}/people/leave`, icon: 'leave' },
      { label: 'Lifecycle', href: `/${slug}/people/lifecycle`, icon: 'shield' },
      { label: 'Departments', href: `/${slug}/people/departments`, icon: 'org' },
      { label: 'Work locations', href: `/${slug}/people/work-locations`, icon: 'company' },
      { label: 'Requests', href: `/${slug}/people/requests`, icon: 'task' },
      { label: 'Shifts', href: `/${slug}/people/shifts`, icon: 'calendar' },
      { label: 'Holidays', href: `/${slug}/people/holidays`, icon: 'calendar' },
      { label: 'Documents', href: `/${slug}/people/documents`, icon: 'document' },
      { label: 'People reports', href: `/${slug}/people/reports`, icon: 'report' },
      { label: 'HR policy', href: `/${slug}/people/settings`, icon: 'settings' },
    ] }];
    const sales: Section[] = [
      { label: 'My day', items: [
      ...
      { label: 'Engage', items: [
        ...
        { label: 'Communications', href: `/${slug}/sales/communications`, icon: 'activity', permission: 'communications' },
      ] },
      { label: 'Grow', items: [
        { label: 'Forms', href: `/${slug}/sales/forms`, icon: 'document', permission: 'forms' },
        { label: 'Landing pages', href: `/${slug}/sales/landing-pages`, icon: 'document', permission: 'landingpages' },
    ...
    return [common, ...sales]
      .map((section) => ({ ...section, items: section.items.filter((item) => !item.permission || permitted.includes(item.permission)) }))
      .filter((section) => section.items.length > 0);
  }, [activeModule, permitted, slug]);

  const administration: Item[] = [
    { label: 'Company profile', href: `/${slug}/admin/company`, icon: 'company' },
    { label: 'Users', href: `/${slug}/admin/users`, icon: 'people' },
    { label: 'Roles & permissions', href: `/${slug}/admin/roles`, icon: 'shield' },
    { label: 'Modules', href: `/${slug}/admin/modules`, icon: 'settings' },
    { label: 'Subscription', href: `/${slug}/admin/subscription`, icon: 'document' },
    { label: 'Integrations', href: `/${slug}/admin/integrations`, icon: 'activity' },
    { label: 'Security', href: `/${slug}/admin/security`, icon: 'shield' },
    { label: 'My security', href: `/${slug}/profile/security`, icon: 'shield' },
    { label: 'Settings', href: `/${slug}/admin/settings`, icon: 'settings' },
    { label: 'Audit logs', href: `/${slug}/admin/audit`, icon: 'report' },
  ];

  return <>
    <button className="lf-mobile-nav-button" onClick={() => setMobileOpen((value) => !value)} aria-label="Open navigation" aria-expanded={mobileOpen}>☰</button>
    <aside className="lf-workspace-sidebar" data-collapsed={collapsed} data-mobile-open={mobileOpen}>
      <div className="lf-sidebar-brand">
        <Link href={`/${slug}/dashboard`} className="lf-brand-mark" aria-label={`${name} dashboard`}>{initials(name)}</Link>
        {!collapsed && <div className="lf-brand-copy">
          <strong>{name}</strong>
          {workspaces.length > 1 ? <select value={slug} aria-label="Switch workspace" onChange={(event) => router.push(`/${event.target.value}/dashboard`)} style={{ width: '100%', marginTop: 2, border: 0, padding: 0, background: 'transparent', color: 'rgb(255 255 255 / .48)', fontSize: 10 }}>
            {workspaces.map((workspace) => <option key={workspace.slug} value={workspace.slug}>{workspace.name}</option>)}
          </select> : <span>{PRODUCT_NAME}</span>}
        </div>}
      ...
      {!collapsed && <div className="lf-module-switch" aria-label="Product modules">
        {modules.includes('SALES') && <Link href={`/${slug}/sales`} aria-current={activeModule === 'sales' ? 'page' : undefined}><Icon name="lead" />Sales</Link>}
        {modules.includes('HRMS') && <Link href={`/${slug}/people`} aria-current={activeModule === 'people' ? 'page' : undefined}><Icon name="people" />People</Link>}
      </div>}

      <nav className="lf-sidebar-nav" aria-label="Workspace">
        {sections.map((section) => <section className="lf-nav-section" key={section.label}>
          {!collapsed && <div className="lf-nav-label">{section.label}</div>}
          {section.items.map((item) => <NavLink key={item.href} item={item} pathname={pathname} collapsed={collapsed} />)}
        </section>)}
        <section className="lf-nav-section">
          {!collapsed && <div className="lf-nav-label">Administration</div>}
          {administration.map((item) => <NavLink key={item.href} item={item} pathname={pathname} collapsed={collapsed} />)}
        </section>
      </nav>

      <div className="lf-sidebar-user">
        <span className="lf-avatar" style={{ background: 'rgb(255 255 255 / .11)', color: '#fff' }}>{initials(user.name)}</span>
        {!collapsed && <div className="lf-sidebar-user-copy"><strong>{user.name}</strong><span>{user.role.replaceAll('_', ' ')}</span></div>}
      </div>
    </aside>
  </>;
}

function NavLink({ item, pathname, collapsed }: { item: Item; pathname: string; collapsed: boolean }) {
  const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
  return <Link className="lf-nav-link" href={item.href} aria-current={active ? 'page' : undefined} title={collapsed ? item.label : undefined}>
    <Icon name={item.icon} />{!collapsed && item.label}
  </Link>;
}
...
function Icon({ name }: { name: IconName }) {
  ...
  return <svg className="lf-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={paths[name]} /></svg>;
}

const initials = (value: string) => value.split(/\s+/).slice(0, 2).map((part) => part[0]?.toUpperCase() ?? '').join('');
