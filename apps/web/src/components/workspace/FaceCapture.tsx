...
export default function FaceCapture({ mode, endpointBase, employeeId, samplesRequired = 4 }: {
  mode: Mode;
  ...
  async function start() {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setError('This browser will not give the page a camera. Browsers only allow camera access over HTTPS or on localhost — reaching this app on a plain http:// LAN address blocks it no matter how the server is configured.');
      return;
    }
    try {
  ...
  function position(): Promise<GeolocationPosition> {
    return new Promise((resolve, reject) => {
      if (!navigator.geolocation) { reject(new Error('This browser will not share a location.')); return; }
      navigator.geolocation.getCurrentPosition(resolve, (cause) => reject(new Error(`Location unavailable: ${cause.message}`)), {
  ...
  async function doPunch(punchType: 'CHECK_IN' | 'CHECK_OUT') {
    setPhase('working'); setError(null); setStatus(null);
    try {
      ...
      if (!check.ok) { setError(check.message); setPhase('ready'); return; }
      ...
      const result = await post('attendance-punch', {
        punchType, nonce: challenge.nonce, frames: [neutral, moved], ...where,
      ...
      if (result.result === 'ACCEPTED' || result.result === 'FLAGGED_REVIEW') {
        setStatus(result.message); setPhase('done'); stop(); router.refresh();
      } else {
        setError(result.message); setPhase('ready');
      }
    } catch (cause) {
      setError((cause as Error).message); setPhase('ready');
    }
  }
  ...
  async function doEnrol() {
    ...
    try {
      ...
      const prompts = ['Look straight at the camera.', 'Turn slightly to your left.', 'Turn slightly to your right.', 'Tilt your head slightly up.'];
      const frames: string[] = [];
      ...
      setInstruction(null); setSamples(frames.length);
      ...
      setPhase('done'); stop(); router.refresh();
    } catch (cause) {
      setError((cause as Error).message); setPhase('ready'); setInstruction(null); setSamples(0);
    }
  }

  return <div style={{ display: 'grid', gap: 12 }}>
    <div style={{ position: 'relative', maxWidth: 420, aspectRatio: '4 / 3', background: '#000', borderRadius: 12, overflow: 'hidden' }}>
      <video ref={videoRef} playsInline muted style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scaleX(-1)' }} />
      {instruction && <div role="status" style={{ position: 'absolute', inset: 'auto 0 0 0', padding: '10px 12px', background: 'rgb(0 0 0 / .68)', color: '#fff', fontWeight: 600, textAlign: 'center' }}>{instruction}</div>}
      {phase === 'idle' && <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', color: 'rgb(255 255 255 / .6)', fontSize: 13 }}>Camera off</div>}
    </div>

    {status && <div role="status" style={{ color: 'var(--lf-ink-600)', fontSize: 'var(--lf-text-sm)' }}>{status}</div>}
    {error && <div className="lf-alert" role="alert">{error}</div>}
    {mode === 'enrol' && samples > 0 && phase === 'working' && <div style={{ fontSize: 'var(--lf-text-xs)', color: 'var(--lf-ink-500)' }}>Sample {samples} of {Math.max(samplesRequired, 4)}</div>}

    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
      {phase === 'idle' && <button className="lf-btn" type="button" onClick={() => void start()}>Start camera</button>}
      {phase === 'ready' && mode === 'punch' && <>
        <button className="lf-btn" type="button" onClick={() => void doPunch('CHECK_IN')}>Check in</button>
        <button className="lf-btn lf-btn--secondary" type="button" onClick={() => void doPunch('CHECK_OUT')}>Check out</button>
        <button className="lf-btn lf-btn--ghost" type="button" onClick={stop}>Stop camera</button>
      </>}
      {phase === 'ready' && mode === 'enrol' && <>
        <button className="lf-btn" type="button" onClick={() => void doEnrol()} disabled={!employeeId}>Capture {Math.max(samplesRequired, 4)} samples</button>
        <button className="lf-btn lf-btn--ghost" type="button" onClick={stop}>Stop camera</button>
      </>}
      {phase === 'working' && <button className="lf-btn" type="button" disabled>Working…</button>}
      {phase === 'done' && <button className="lf-btn lf-btn--ghost" type="button" onClick={() => setPhase('idle')}>Done</button>}
    </div>
  </div>;
}
