...
const LABEL: Record<Scope, string> = {
  NONE: 'No access', OWN: 'Own records', TEAM: 'Their team',
  BRANCH: 'Their branch', REGION: 'Their region', ORGANIZATION: 'Whole workspace',
};
 ...
 */
export default function PermissionMatrix({ endpoint, roleId, editable, rows }: {
  endpoint: string;
  roleId: string;
  editable: boolean;
  rows: MatrixRow[];
  ...
  async function save() {
    ...
    try {
      const response = await authFetch(`${endpoint}/matrix-update`, {
        ...
        body: JSON.stringify({
          roleId,
          changes: dirty.map((row) => ({ permissionId: row.permissionId, granted: values[row.permissionId] !== 'NONE', scope: values[row.permissionId] })),
        }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) { setError(data.detail ?? 'The permissions could not be saved.'); return; }
      setNotice(`Saved ${data.changed} change${data.changed === 1 ? '' : 's'}. ${data.sessionsRevoked} session${data.sessionsRevoked === 1 ? '' : 's'} signed out so the new permissions take effect.`);
      router.refresh();
    } catch { setError('The server could not be reached.'); }
    finally { setBusy(false); }
  }

  return <div style={{ display: 'grid', gap: 'var(--lf-space-4)' }}>
    {error && <div className="lf-alert" role="alert">{error}</div>}
    {notice && <div role="status" style={{ color: 'var(--lf-ink-600)' }}>{notice}</div>}

    {grouped.map(([module, moduleRows]) => <section className="lf-card" key={module} style={{ padding: 'var(--lf-space-5)' }}>
      <h3 style={{ margin: '0 0 10px', fontSize: 'var(--lf-text-md)', textTransform: 'capitalize' }}>{module.replace(/_/g, ' ')}</h3>
      <div style={{ display: 'grid', gap: 8 }}>
        {moduleRows.map((row) => {
          const ceiling = ORDER.indexOf(row.maxScope);
          return <div key={row.permissionId} style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
            ...
            <select
              ...
              onChange={(event) => setValues((current) => ({ ...current, [row.permissionId]: event.target.value as Scope }))}
            >
              {ORDER.map((option, index) => <option key={option} value={option} disabled={index > ceiling}>
                {LABEL[option]}{index > ceiling ? ' — above your own level' : ''}
              </option>)}
            </select>
          </div>;
        })}
    ...
    {editable && <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      {dirty.length > 0 && <button className="lf-btn lf-btn--ghost" type="button" onClick={() => setValues(Object.fromEntries(rows.map((row) => [row.permissionId, row.granted ? row.scope : 'NONE'])))}>Discard</button>}
