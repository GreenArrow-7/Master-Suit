 ...
 */
export default function TemporaryPasswordButton({
  endpoint,
  userId,
  userName,
}: {
  ...
  async function reset() {
    if (
      !window.confirm(
        `Reset the password for ${userName}? Every session they have open will be signed out, and they must set their own password before anything else opens.`,
      )
    )
      return;
    setBusy(true);
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.detail ?? 'The password could not be reset.');
        return;
      }
    ...
    return (
      <div className="lf-alert" style={{ display: 'grid', gap: 8 }}>
        ...
        <span style={{ fontSize: 'var(--lf-text-sm)' }}>
          Shown once — it is not stored in readable form and cannot be retrieved. Hand it over in person or by phone,
          not by email or chat.
        </span>
        <div>
          <button className="lf-btn lf-btn--ghost" type="button" onClick={() => setIssued(null)}>
            Done
          </button>
        </div>
      </div>
  ...
  return (
    <span style={{ display: 'inline-grid', gap: 4 }}>
      <button className="lf-btn lf-btn--secondary" type="button" disabled={busy} onClick={reset}>
