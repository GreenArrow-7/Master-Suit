...
export default function SecurityPanel({
  ...
  async function changePassword(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await call('password-change', {
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--lf-space-6)' }}>
      ...
      <section className="lf-card" style={{ padding: 'var(--lf-space-5)' }}>
        ...
        <form onSubmit={changePassword} className="lf-form-grid" style={{ maxWidth: 460 }}>
          <label className="lf-field">
            <span className="lf-label">Current password</span>
            <input
              className="lf-input"
          ...
          <div>
            <button className="lf-btn" type="submit" disabled={busy === 'password-change'}>
              {busy === 'password-change' ? 'Saving…' : 'Change password'}
            </button>
          </div>
        </form>
      </section>
      ...
      <section className="lf-card" style={{ padding: 'var(--lf-space-5)', display: 'grid', gap: 10 }}>
        ...
        <div>
          <button
            ...
            onClick={async () => {
              if (!window.confirm('Sign out of every device, including this one? You will need to sign in again.'))
                return;
              setBusy('logout-all');
              setError(null);
              try {
      ...
      <section className="lf-card" style={{ padding: 'var(--lf-space-5)', display: 'grid', gap: 12 }}>
        ...
        {enrolment && !status.enabled && (
          <div style={{ display: 'grid', gap: 10 }}>
            <p style={{ margin: 0, fontSize: 'var(--lf-text-sm)' }}>
              Add this key to your authenticator app, then enter the six-digit code it shows. Two-factor authentication
              is <strong>not active</strong> until you confirm a code — so a mistyped key cannot lock you out.
            </p>
            <code
              style={{
                display: 'block',
            ...
            >
              <label className="lf-field" style={{ maxWidth: 160 }}>
                <span className="lf-label">Code from your app</span>
                <input
                  className="lf-input"
                  name="code"
                  inputMode="numeric"
              ...
              <button className="lf-btn" type="submit" disabled={busy === 'two-factor-confirm'}>
                Confirm and turn on
              </button>
              <button className="lf-btn lf-btn--ghost" type="button" onClick={() => setEnrolment(null)}>
                Cancel
              </button>
            </form>
          </div>
        ...
        {recoveryCodes && (
          <div className="lf-alert" style={{ display: 'grid', gap: 8 }}>
            <strong>Save these recovery codes now.</strong>
            <span style={{ fontSize: 'var(--lf-text-sm)' }}>
              Each works once. They are shown only here, are not stored in readable form, and are the only way back into
              your account if you lose the authenticator.
            </span>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 6 }}>
              {recoveryCodes.map((code) => (
                <code key={code} style={{ padding: '6px 8px', background: 'var(--lf-surface-2)', borderRadius: 6 }}>
                  {code}
                </code>
              ))}
            </div>
            <div>
              <button className="lf-btn lf-btn--ghost" type="button" onClick={() => setRecoveryCodes(null)}>
                I have saved them
              </button>
            </div>
          </div>
        ...
        {status.enabled && !recoveryCodes && (
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            >
              <label className="lf-field" style={{ maxWidth: 160 }}>
                <span className="lf-label">Code from your app</span>
                <input className="lf-input" name="code" inputMode="numeric" pattern="\d{6}" maxLength={6} required />
              </label>
              <button
                className="lf-btn lf-btn--secondary"
                type="submit"
                disabled={busy === 'two-factor-recovery-regenerate'}
              >
            ...
            <details>
              ...
              <form
                style={{ display: 'flex', gap: 8, alignItems: 'end', flexWrap: 'wrap', marginTop: 10 }}
                onSubmit={async (event) => {
                  event.preventDefault();
              ...
              >
                <label className="lf-field" style={{ maxWidth: 220 }}>
                  <span className="lf-label">Your password</span>
                  <input
                    className="lf-input"
                    name="password"
                    type="password"
                ...
                <button className="lf-btn lf-btn--danger" type="submit" disabled={busy === 'two-factor-disable'}>
                  Turn off
                </button>
              </form>
