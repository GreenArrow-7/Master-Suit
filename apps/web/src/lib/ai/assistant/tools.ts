...
export const TOOLS: ToolDef[] = [
...

  {
    ...
    execute: async (ctx, args) => {
      // The one write-shaped surface that reaches configuration rather than a
      // record, so it demands the automation permission, not a record grant.
      if (!can(ctx, 'automation', 'MANAGE_AUTOMATION')) return denied('automation management');
