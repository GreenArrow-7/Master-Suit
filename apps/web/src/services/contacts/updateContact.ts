...
export async function updateContact(ctx: Ctx, id: string, input: UpdateContactInput) {
  const updated = await withTenantTx(ctx.tenantId, async (tx) => {
    ...
    await assertRecordVisible(ctx, 'contacts', before, 'EDIT');
...
export async function deleteContact(ctx: Ctx, id: string) {
  await withTenantTx(ctx.tenantId, async (tx) => {
    const before = await tx.contact.findFirst({ where: { tenantId: ctx.tenantId, id } });
    if (!before) throw NotFound('Contact');
    await assertRecordVisible(ctx, 'contacts', before, 'DELETE');
    await tx.contact.update({ where: { tenantId: ctx.tenantId, id }, data: { deletedAt: new Date(), updatedById: ctx.actor.id } });
