...
export const HR_SETTINGS: Definition[] = [
  // ── Face matching ────────────────────────────────────────────────────────
  {
    ...
    group: 'face',
    type: 'number',
    default: env.FACE_MATCH_THRESHOLD,
    min: 0.3,
    max: 0.95,
    step: 0.01,
  ...
  {
    key: 'faceSamplesRequired',
    group: 'face',
    type: 'number',
    default: env.FACE_SAMPLES_REQUIRED,
    min: 1,
    max: 10,
    step: 1,
    unit: 'samples',
  ...
  {
    key: 'faceEnrolmentMinSpread',
    group: 'face',
    type: 'number',
    default: env.FACE_ENROLMENT_MIN_SPREAD,
    min: 0,
    max: 0.5,
    step: 0.005,
  ...
  {
    key: 'livenessSamePersonThreshold',
    group: 'liveness',
    type: 'number',
    default: 0.4,
    min: 0.1,
  ...
  // ── Attendance ───────────────────────────────────────────────────────────
  {
    ...
    group: 'attendance',
    type: 'number',
    default: env.MAX_GPS_ACCURACY_M,
    min: 5,
    max: 1000,
    step: 5,
    unit: 'm',
  ...
  {
    key: 'minPunchIntervalSeconds',
    group: 'attendance',
    type: 'number',
    default: env.MIN_PUNCH_INTERVAL_SECONDS,
    min: 0,
    max: 3600,
    step: 10,
    unit: 's',
  ...
  {
    key: 'maxOfflineSyncHours',
    group: 'attendance',
    type: 'number',
    default: env.MAX_OFFLINE_SYNC_HOURS,
    min: 1,
    max: 336,
    step: 1,
    unit: 'h',
  ...
  {
    key: 'captureRetentionDays',
    group: 'attendance',
    type: 'number',
    default: env.ATTENDANCE_CAPTURE_RETENTION_DAYS,
    min: 1,
    max: 3650,
    step: 1,
    unit: 'days',
  ...
  {
    key: 'accrualDaysPerMonthUnderYear',
    group: 'leave',
    type: 'number',
    default: 2,
    min: 0,
  ...
  {
    key: 'accrualDaysPerMonthAfterYear',
    group: 'leave',
    type: 'number',
    default: 2.5,
    min: 0,
  ...
  {
    key: 'gratuityDaysAfterFirstPeriod',
    group: 'gratuity',
    type: 'number',
    default: 30,
    min: 0,
  ...
  },
  {
    ...
    default: 'total',
    options: [
      { value: 'total', label: 'Total remuneration' },
  ...
  {
    key: 'agentDepartments',
    group: 'lifecycle',
    type: 'strings',
    default: ['brokerage', 'sales', 'leasing'],
...
export type HrPolicy = {
  ...
  faceSamplesRequired: number;
  faceEnrolmentMinSpread: number;
  ...
  noticePayBasis: 'total' | 'basic';
  defaultNoticeDays: number;
  documentExpiryHorizonDays: number;
  agentDepartments: string[];
};

export const DEFAULT_POLICY = Object.fromEntries(
  HR_SETTINGS.map((setting) => [setting.key, setting.default]),
) as HrPolicy;
  ...
  switch (setting.type) {
    case 'number': {
      ...
      if (value < setting.min || value > setting.max)
        return { ok: false, message: `must be between ${setting.min} and ${setting.max}` };
...
export async function updateHrPolicy(ctx: Ctx, patch: Record<string, unknown>) {
  ...
  for (const setting of HR_SETTINGS) {
    ...
    if (!parsed.ok) {
      errors.push({ field: setting.key, code: 'invalid', message: `${setting.label} ${parsed.message}.` });
      continue;
    }
