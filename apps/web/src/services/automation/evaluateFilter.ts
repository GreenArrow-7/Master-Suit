...
export function evaluateFilterNode(node: FilterNode, record: Record<string, any>): boolean {
  ...
  switch (node.cmp) {
    ...
    case 'contains': return Array.isArray(actual) ? actual.includes(value) : String(actual ?? '').toLowerCase().includes(String(value).toLowerCase());
    case 'starts': return String(actual ?? '').toLowerCase().startsWith(String(value).toLowerCase());
    case 'ends': return String(actual ?? '').toLowerCase().endsWith(String(value).toLowerCase());
    case 'between': { const [a, b] = value as [any, any]; return actual >= a && actual <= b; }
