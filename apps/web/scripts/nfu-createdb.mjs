...
if (!src) { console.error('MIGRATION_DATABASE_URL missing'); process.exit(1); }
const u = new URL(src);
const target = process.argv[2] ?? 'master_suite_nfu';
const admin = new URL(src); admin.pathname = '/postgres'; admin.search = '';
...
else { await c.query(`CREATE DATABASE "${target}" OWNER "${u.username}"`); console.log(`created ${target} owner=${u.username}`); }
await c.end();
