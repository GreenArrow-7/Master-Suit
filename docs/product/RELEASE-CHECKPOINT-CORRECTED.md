...
> **Superseded on 12 September 2026 by [RELEASE-CHECKPOINT-2026-09-12.md](RELEASE-CHECKPOINT-2026-09-12.md)** for the gate table, the booking evidence map, the collections position and the operator work. This file remains the record of the runs and corrections it describes.
...
container's database configuration, through a bind mount — _while a gate run was
using it_, which produced a 91-file failure that looked like a code regression
...
deliberately rather than deleted. It reports _91 files failed, 1,252 skipped_ at
...
first reported _1 failed, 4 skipped_; the second _2 failed, 0 skipped_. The two
...
- `credential-purge` asserts `buildId()` falls back to a git SHA in a source
  ...
  pointer file holding a _Windows_ absolute path; that path cannot resolve
...
failing run reported _1 failed, 2,144 passed_, and a **different** test had
...
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docs/product/IMPLEMENTATION-BACKLOG.md`   | Already specifies the fix: _"Partial unique index on `Booking(unitInventoryId)` where `status = 'CONFIRMED' AND deletedAt IS NULL`"_, plus the files, the backfill risk and six acceptance tests. The invariant is keyed on the **unit**. |
| `docs/product/WORKFLOW-BLUEPRINT.md` §H    | _"Confirmation must, in one transaction: verify the unit is reservable by this agent, move it, and write the booking."_ Presupposes a unit.                                                                                               |
| `docs/product/CURRENT-CODE-MAP.md`         | Records the defect as **reproduced**, not merely inspected: _"two concurrent confirms both returned 200 **[R]**"_, and _"**no** unique index on `unitInventoryId`"_.                                                                      |
...
matches. **None of them states whether a booking _without_ a unit may be
...
> what stops _that_ being double-sold is then an open question rather than a
...
**What it does NOT settle.** A _draft_ may still be vague about what is being
...
records _how much_ arrived rather than only _that_ something did, is a separate
...
1. Lock the unit, not the booking. Two _different_ draft bookings on one unit is
...
| _(added)_ Confirming a booking that names no unit → refused                           | PASS — the policy above                                                                  |
| _(added)_ The database rejects a confirmed booking with no unit when bypassed         | PASS                                                                                     |
...
collection authority and the missing collected _amount_, which no one has
...
the _permission_ does not remove existing bookings or commissions, so P&L,
...
Plus three negative controls, so the restriction is proved to be _narrow_:
...
| **Payroll cost attributed to the employee's _current_ team**       | **OPEN**  | `pl.ts:236` reads `salesUser.branchId / regionId / teams` live. A transfer restates historical payroll cost — exactly the failure the booking side was fixed to avoid. |
...
>
> **C2 — Amount.** Must the system record _how much_ was collected (and against
